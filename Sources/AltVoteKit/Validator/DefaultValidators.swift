extension Validator{
	...
	public static let oneVotePerUser = Validator(name: "One vote per. user", offenseText: {"\($0.userID) voted multiple times"}) { votes, _  in
	...
	public static let everyoneHasVoted = Validator(name: "Everyone has voted", offenseText: {"\($0.userID) hasn't voted"}) { votes, eligibleUsers in
	...
	public static let noForeignVotes = Validator(name: "No foreign votes", offenseText: {"\($0.userID) has voted enough they aren't on the list of allowed users"}) { votes, eligibleUsers in
	...
	public static let preferenceForAllCandidates = Validator(name: "All candidates requires a vote", offenseText: {"\($0.userID) hasn't voted for all candidates"}) {
		votes, _, options in
	...
	public static let noBlankVotes = Validator(name: "No blank votes", offenseText: {"\($0.userID) voted blank"}) { votes, _ in
